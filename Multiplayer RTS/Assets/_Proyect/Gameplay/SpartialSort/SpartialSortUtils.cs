...
{    
    /// <summary>
    /// Is sorts the points based on what is their current standing hex.
    /// </summary>
    ...
    public static Dictionary<Hex, List<EntityOnVision>> GetHexToPointsDictionary(List<EntityOnVision> allPoints)
    {
        int pointCount = allPoints.Count;
        var cellPoints = new Dictionary<Hex, List<EntityOnVision>>(pointCount);

        foreach (var currEntity in allPoints)
        {
            var currHex = currEntity.position.Round();

            List<EntityOnVision> pointsOfHex;
            if (cellPoints.TryGetValue(currHex, out pointsOfHex))
            {
                pointsOfHex.Add(currEntity);
                cellPoints[currHex] = pointsOfHex;
            }
            else
            {
                pointsOfHex = new List<EntityOnVision>() { currEntity };
                cellPoints.Add(currHex, pointsOfHex);
            }
    ...
    public static List<EntityOnVision> GetAllPointsAtRange(Hex hex, int range, Dictionary<Hex, List<EntityOnVision>> entitiesForeachHex)
    {
        var pointsAtRange = new List<EntityOnVision>();

        var cellsToCheck = Hex.AllHexesInRange(hex, range, true);

        foreach (Hex neightbor in cellsToCheck)
        {
            if (entitiesForeachHex.TryGetValue(neightbor, out List<EntityOnVision> neightborEntities))
            {
                pointsAtRange.AddRange(neightborEntities);
            }
        }
